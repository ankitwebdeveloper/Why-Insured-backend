import db from './db.js';

export function seedDatabase() {
  const now = new Date().toISOString();

  // 1. Seed Plan Information
  const existingPlan = db.plans.findById('hdfc-optima-secure-plus');

  if (!existingPlan) {
    db.plans.upsert({
      id: 'hdfc-optima-secure-plus',
      company_name: 'HDFC ERGO',
      plan_name: 'Optima Secure+',
      tagline: 'Unlimited Protection. Added Every Year.',
      policy_subtitle: 'HDFC ERGO Health Insurance Policy',
      description: '4X Coverage with Secure, Plus, Restore & Protect benefits and unlimited restoration.',
      logo: '/assets/hdfc-ergo.png',
      coverage: '₹10 Lakh - ₹2 Crore',
      status: 'active'
    });
    console.log('Seeded HDFC ERGO Optima Secure+ plan metadata.');
  }

  // 2. Check and Seed Features
  const features = db.planFeatures.findByPlanId('hdfc-optima-secure-plus', { includeInactive: true });

  if (features.length === 0) {
    const initialFeatures = [
      // MOST IMPORTANT FEATURES
      {
        id: 's1-1',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'most_important',
        title: 'Any Room Category',
        subtitle: '100% Cashless Policy with zero room rent capping',
        summary: 'Any Room Category (Single, Twin, Suite, etc.) is covered under 100% Cashless Policy with no proportionate deductions or sub-limits.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: '100% CASHLESS',
        icon_type: 'home',
        video_title: 'Any Room Category',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's1-2',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'most_important',
        title: 'No Limit on ICU, etc.',
        subtitle: 'Zero capping on Intensive Care Unit (ICU) charges',
        summary: 'No limit on ICU charges, monitoring equipment, specialist consultations, and associated critical care expenses.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'NO ICU LIMIT',
        icon_type: 'activity',
        video_title: 'No Limit on ICU, etc.',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's1-3',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'most_important',
        title: 'Pre & Post Hospitalization',
        subtitle: 'Up to 60 days before admission & 180 days after discharge',
        summary: 'Covers medical expenses such as doctor consultations, pharmacy/medicine expenses, lab tests, etc. Pre-hospitalization covers up to 60 days before admission and post-hospitalization covers up to 180 days after discharge.',
        detailed_description: '',
        intro: '',
        points: [
          '**Covers medical expenses:** Covers medical expenses such as doctor consultations, pharmacy/medicine expenses, lab tests, etc.',
          '**Pre-Hospitalisation:** Up to **60 days before admission**',
          '**Post-Hospitalisation:** Up to **180 days after discharge**'
        ],
        steps: null,
        badge: '60 & 180 DAYS',
        icon_type: 'calendar',
        video_title: 'Pre & Post Hospitalization',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's1-4',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'most_important',
        title: 'All Day Care Diseases covered',
        subtitle: 'All Day Care Treatments Covered ',
        summary: 'Covers all day care treatments that require less than 24 hours of hospitalization.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'ALL DAY CARE',
        icon_type: 'check',
        video_title: 'All Day Care Diseases covered',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 4,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's1-5',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'most_important',
        title: 'Modern Treatment & Robotic Surgery',
        subtitle: 'Modern & Robotic Treatments',
        summary: 'Modern treatments and robotic surgeries are covered as per policy terms.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'ROBOTIC SURGERY',
        icon_type: 'cpu',
        video_title: 'Modern Treatment & Robotic Surgery',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 5,
        status: 'active',
        created_at: now,
        updated_at: now
      },

      // VALUE ADDED FEATURES
      {
        id: 's2-1',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'value_added',
        title: 'Unlimited Restoration',
        subtitle: '100% Base Sum Insured Restored Automatically',
        summary: '100% Base Sum Insured restored automatically unlimited times for subsequent related or unrelated claims within the policy year.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: ['10 Lakh Base SI', '10 Lakh Restored', '10 Lakh Restored Again', 'Unlimited Times'],
        badge: 'UNLIMITED TIMES',
        icon_type: 'refresh',
        video_title: 'Unlimited Restoration',
        video_url: 'asset:unlimited',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's2-2',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'value_added',
        title: 'Infinite Benefit*',
        subtitle: '100% Base SI Added Every Year (Irrespective of claims)',
        summary: '100% Base Sum Insured is added every year irrespective of claims: 20 Lakh → 40 Lakh → 60 Lakh → ...... infinite times.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: ['20 Lakh', '40 Lakh', '60 Lakh', '...... infinite times'],
        badge: '100% BASE SI EVERY YEAR',
        icon_type: 'trending',
        video_title: 'Infinite Benefit*',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's2-3',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'value_added',
        title: 'Secure Benefit: 2X',
        subtitle: '2X cover from Day 1',
        summary: 'Instantly doubles your sum insured from day 1 without paying any extra premium.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: ['Base SI: 20 Lac', '20 Lac + 20 Lac = 40 Lac', '2X cover from Day 1'],
        badge: '2X FROM DAY 1',
        icon_type: 'shield',
        video_title: 'Secure Benefit: 2X',
        video_url: 'asset:2x_coverage',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's2-4',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'value_added',
        title: 'Protect Benefit',
        subtitle: 'Cover non-medical expenses (Consumables)',
        summary: 'Covers non-medical expenses and consumables like gloves, cotton, syringes, masks, and other listed medical consumables.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'NON-MEDICAL COVER',
        icon_type: 'clipboard',
        video_title: 'Protect Benefit',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 4,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's2-5',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'value_added',
        title: 'Preventive Health Check-up',
        subtitle: 'Covered once in every policy year',
        summary: 'Covered once in every policy year for all insured members on a reimbursement basis, subject to policy terms.',
        detailed_description: '',
        intro: '',
        points: [
          '**Covered once** in every policy year',
          '**Available for all insured members**, subject to policy terms',
          '**Health check-up is available** on a reimbursement basis',
          '**The applicable limit** depends on the Sum Insured selected'
        ],
        steps: null,
        badge: 'ANNUAL BENEFIT',
        icon_type: 'smile',
        video_title: 'Preventive Health Check-up',
        video_url: 'asset:preventive',
        has_health_checkup_table: 1,
        health_checkup_limits: [
          { baseSI: '₹10 Lakh', floater: '₹5,000', individual: '₹2,000' },
          { baseSI: '₹15 Lakh', floater: '₹8,000', individual: '₹4,000' },
          { baseSI: '₹20–50 Lakh', floater: '₹10,000', individual: '₹5,000' },
          { baseSI: '₹100–200 Lakh', floater: '₹15,000', individual: '₹8,000' }
        ],
        table_button_label: 'View Health Check-up Limits',
        display_order: 5,
        status: 'active',
        created_at: now,
        updated_at: now
      },

      // ADDITIONAL FEATURES
      {
        id: 's3-1',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'additional',
        title: 'Daily Cash for Shared Room',
        subtitle: '₹800 per day up to a maximum of ₹4,800',
        summary: 'Receive daily cash allowance of ₹800 per day up to a maximum of ₹4,800 when you choose shared accommodation in a network hospital and hospitalization exceeds 48 hours.',
        detailed_description: '',
        intro: 'Receive daily cash allowance of **₹800 per day** up to a maximum of **₹4,800** when:',
        points: [
          '**You choose shared accommodation in a network hospital**',
          '**Hospitalisation exceeds 48 hours**'
        ],
        steps: ['₹800/day × maximum 6 days = ₹4,800 maximum'],
        badge: 'DAILY CASH',
        icon_type: 'dollar',
        video_title: 'Daily Cash for Shared Room',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's3-2',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'additional',
        title: 'Domiciliary, Organ & AYUSH Treatment',
        subtitle: 'Home, Organ Donor & AYUSH Care',
        summary: 'Domiciliary Hospitalisation: Treatment at home is covered as per policy terms. Organ Donor Expenses: Covers hospitalization expenses of the organ donor for organ harvesting, up to the Sum Insured. AYUSH Treatment: In-patient treatment under AYUSH systems like Ayurveda, Yoga & Naturopathy, Unani, Siddha, and Homeopathy is covered, up to the Sum Insured.',
        detailed_description: '',
        intro: '',
        points: [
          '**Domiciliary Hospitalisation:** Treatment at home is covered as per policy terms.',
          '**Organ Donor Expenses:** Covers hospitalization expenses of the organ donor for organ harvesting, up to the Sum Insured.',
          '**AYUSH Treatment:** In-patient treatment under AYUSH systems like Ayurveda, Yoga & Naturopathy, Unani, Siddha, and Homeopathy is covered, up to the Sum Insured.'
        ],
        steps: null,
        badge: 'AYUSH & ORGAN',
        icon_type: 'activity',
        video_title: 'Domiciliary, Organ & AYUSH Treatment',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's3-3',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'additional',
        title: 'Emergency Ambulance Cover',
        subtitle: 'Road & Air Ambulance',
        summary: 'Emergency ambulance transportation is covered. Road: Up to Sum Insured. Air: Up to INR 5,00,000.',
        detailed_description: '',
        intro: '',
        points: [
          '**Emergency ambulance transportation is covered**',
          '**Road:** Up to Sum Insured',
          '**Air:** Up to INR 5,00,000'
        ],
        steps: null,
        badge: 'ROAD & AIR AMBULANCE',
        icon_type: 'truck',
        video_title: 'Emergency Ambulance Cover',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      },

      // OPTIONAL RIDERS (ADD-ONS)
      {
        id: 's4-1',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'optional_rider',
        title: 'ABCD Chronic Care',
        subtitle: 'Pre-existing diseases covered from 31st day',
        summary: 'Covers hospitalisation expenses for Asthma, Blood pressure, Cholesterol and Diabetes from the 31st day',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'CHRONIC CARE',
        icon_type: 'heart',
        video_title: 'ABCD Chronic Care',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's4-2',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'optional_rider',
        title: 'Optima Wellbeing',
        subtitle: 'Information abhi mentioned karni hai with limits ke sath',
        summary: 'Information abhi mentioned karni hai with limits ke sath.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'WELLBEING',
        icon_type: 'smile',
        video_title: 'Optima Wellbeing',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's4-3',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'optional_rider',
        title: 'Limitless',
        subtitle: 'One unlimited claim in a lifetime',
        summary: 'Get one unlimited claim in a lifetime, with no Sum Insured limit.',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'LIMITLESS',
        icon_type: 'zap',
        video_title: 'Limitless',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's4-4',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'optional_rider',
        title: 'Parenthood',
        subtitle: 'Maternity, Embryo Storage & IVF',
        summary: 'Covers maternity expenses, embryo storage costs and IVF treatments',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: 'MATERNITY',
        icon_type: 'users',
        video_title: 'Parenthood',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 4,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 's4-5',
        plan_id: 'hdfc-optima-secure-plus',
        section: 'optional_rider',
        title: 'Serious Illness Booster',
        subtitle: '9 Critical Illnesses Covered',
        summary: 'Covers 9 listed critical illnesses',
        detailed_description: '',
        intro: '',
        points: null,
        steps: null,
        badge: '9 CRITICAL ILLNESSES',
        icon_type: 'shield',
        video_title: 'Serious Illness Booster',
        video_url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
        has_health_checkup_table: 0,
        health_checkup_limits: null,
        table_button_label: '',
        display_order: 5,
        status: 'active',
        created_at: now,
        updated_at: now
      }
    ];

    db.planFeatures.seedInitial(initialFeatures);
    console.log(`Seeded ${initialFeatures.length} initial features for HDFC ERGO Optima Secure+.`);
  }

  // 3. Check and Seed Report Card
  const reportCards = db.reportCard.findByPlanId('hdfc-optima-secure-plus', { includeInactive: true });
  if (reportCards.length === 0) {
    const initialReportCards = [
      {
        id: 'rc-csr',
        plan_id: 'hdfc-optima-secure-plus',
        category: 'Claims',
        title: 'CSR',
        score: '97.8%',
        subtitle: 'Claim Settlement Ratio',
        description: 'CSR shows the percentage of eligible claims that an insurer settles during a given period. A higher ratio generally indicates a strong claim settlement track record.',
        single_year: '97.45% → 97.8%',
        single_year_label: 'Recent Single Year',
        three_year_avg: '96.7% → 97.6%',
        three_year_avg_label: '3 Year Average',
        video_title: 'CSR Metrics',
        video_url: 'asset:preventive',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'rc-icr',
        plan_id: 'hdfc-optima-secure-plus',
        category: 'Claims',
        title: 'ICR',
        score: '86%',
        subtitle: 'Incurred Claim Ratio',
        description: "ICR indicates the proportion of premium earned that an insurer spends on settling claims. It provides an overview of the insurer's claims experience.",
        range: '81% → 86%',
        range_label: 'Incurred Claim Ratio',
        video_title: 'ICR Metrics',
        video_url: 'asset:preventive',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'rc-complaint',
        plan_id: 'hdfc-optima-secure-plus',
        category: 'Customer Service',
        title: 'COMPLAINT VOLUME',
        score: '9.28',
        subtitle: 'Complaints per 10,000 Claims',
        description: 'Complaint volume indicates the number of complaints received in relation to the number of claims handled. It helps provide context about the customer claim experience.',
        value: '4.99 → 9.28',
        label: 'Complaints per 10,000 Claims',
        video_title: 'Complaint Volume Metrics',
        video_url: 'asset:preventive',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      }
    ];
    db.reportCard.seedInitial(initialReportCards);
    console.log(`Seeded ${initialReportCards.length} Report Card items for Optima Secure+.`);
  }

  // 4. Check and Seed Company Strength
  const companyStrengths = db.companyStrength.findByPlanId('hdfc-optima-secure-plus', { includeInactive: true });
  if (companyStrengths.length === 0) {
    const initialCompanyStrengths = [
      {
        id: 'cs-ownership',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'OWNERSHIP / PERCENTAGE',
        value: '51% / 49%',
        description: 'Ownership represents the shareholding structure of HDFC ERGO General Insurance.',
        items: [
          { name: 'HDFC Bank', value: '51%', label: 'Ownership' },
          { name: 'ERGO International AG', value: '49%', label: 'Ownership' }
        ],
        video_title: 'Ownership & Shareholding',
        video_url: 'asset:2x_coverage',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'cs-credit',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'CREDIT RATING',
        value: 'AAA',
        description: 'Credit ratings indicate the financial strength and ability of an insurer to meet its financial obligations.',
        items: [
          { agency: 'CRISIL', rating: 'AAA / Stable' },
          { agency: 'ICRA', rating: 'AAA / Stable' }
        ],
        video_title: 'Credit Ratings & Financial Strength',
        video_url: 'asset:2x_coverage',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'cs-capital',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'CAPITAL STRENGTH',
        value: '2.00×',
        label: 'Solvency (as of March 2025)',
        description: "Solvency indicates the insurer's financial capacity to meet its obligations and support its business.",
        video_title: 'Capital Strength & Solvency',
        video_url: 'asset:unlimited',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'cs-financial',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'FINANCIAL BASE',
        value: '₹27,373 Cr',
        label: 'Investment assets (as of March 2025)',
        description: "Investment assets provide an indication of the financial base supporting the insurer's operations.",
        video_title: 'Financial Base & Investments',
        video_url: 'asset:unlimited',
        display_order: 4,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'cs-reinsurance',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'REINSURANCE STRENGTH',
        value: '85%+',
        label: 'Placed with A+ or higher-rated reinsurers',
        description: 'Reinsurance helps insurers manage large or unexpected claims and strengthens their risk-management capacity.',
        video_title: 'Reinsurance Strength',
        video_url: 'asset:preventive',
        display_order: 5,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'cs-market',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'MARKET POSITION',
        value: '5.3%',
        label: 'GDPI market share (FY2025)',
        description: "GDPI market share indicates the insurer's share of the general insurance premium market.",
        video_title: 'Market Position & Share',
        video_url: 'asset:preventive',
        display_order: 6,
        status: 'active',
        created_at: now,
        updated_at: now
      }
    ];
    db.companyStrength.seedInitial(initialCompanyStrengths);
    console.log(`Seeded ${initialCompanyStrengths.length} Company Strength items for Optima Secure+.`);
  }

  // 5. Check and Seed Limitations
  const limitations = db.limitations.findByPlanId('hdfc-optima-secure-plus', { includeInactive: true });
  if (limitations.length === 0) {
    const initialLimitations = [
      {
        id: 'lim-initial',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'Initial Waiting Period (30 Days)',
        summary: 'An initial waiting period of 30 days applies from the policy inception date for any illness or medical hospitalisation.',
        description: 'An initial waiting period of 30 days applies from the policy inception date for any illness or medical hospitalisation.',
        highlight: 'Accidental hospitalisation is covered from Day 1 with zero waiting period.',
        duration_tag: '30 Days',
        policy_ref: 'HDFC ERGO Optima Secure Policy Terms',
        video_title: 'Initial Waiting Period (30 Days)',
        video_url: 'asset:preventive',
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'lim-specific',
        plan_id: 'hdfc-optima-secure-plus',
        title: '2 Years Waiting Period on Specific Diseases',
        summary: 'A continuous waiting period of 24 months (2 Years) is applicable for medical/surgical treatment of specified illnesses/procedures.',
        description: 'A continuous waiting period of 24 months (2 Years) is applicable for medical/surgical treatment of specified illnesses/procedures.',
        disease_list_header: 'Covered after 24 Months Continuous Coverage',
        disease_list: [
          'Cataract & eye surgeries',
          'Benign Prostatic Hypertrophy (BPH)',
          'Hernia (all types) & Hydrocele',
          'Piles, Fistula & Fissure in ano',
          'Stones in Urinary & Biliary systems',
          'Hysterectomy / Uterine Fibroids',
          'Joint replacements (non-accidental)',
          'Osteoarthritis & Osteoporosis',
          'Sinusitis, DNS, Tonsillectomy',
          'Benign cysts, polyps & tumors',
          'Varicose veins & varicose ulcers',
          'Spondylosis & Disc disorders'
        ],
        duration_tag: '24 Months',
        policy_ref: 'HDFC ERGO Optima Secure Policy Terms (Code: Excl02)',
        video_title: '2 Years Waiting Period on Specific Diseases',
        video_url: 'asset:preventive',
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'lim-permanent',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'Permanent Exclusion',
        summary: 'The policy will not cover expenses incurred towards hospitalisation or treatment arising from permanent exclusions.',
        description: 'The policy will not cover expenses incurred towards hospitalisation or treatment arising from permanent exclusions.',
        exclusions_list_header: 'Permanently Excluded from Coverage',
        exclusions_list: [
          'Cosmetic, aesthetic & plastic surgery',
          'Intentional self-injury & suicide attempt',
          'Alcohol, drug or substance abuse',
          'Obesity & weight control surgeries',
          'Investigation & evaluation admissions',
          'Rest cure, rehabilitation & respite care',
          'Unproven / experimental treatments',
          'Participation in hazardous adventure sports',
          'Expenses arising from breach of law',
          'War, nuclear or chemical contamination'
        ],
        duration_tag: 'Never Covered',
        policy_ref: 'Standard IRDAI & HDFC ERGO Policy Terms (Code: Excl01–Excl18)',
        video_title: 'Permanent Exclusion',
        video_url: 'asset:preventive',
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      }
    ];
    db.limitations.seedInitial(initialLimitations);
    console.log(`Seeded ${initialLimitations.length} Limitations items for Optima Secure+.`);
  }

  // 6. Check and Seed Must Know
  const mustKnowItems = db.mustKnow.findByPlanId('hdfc-optima-secure-plus', { includeInactive: true });
  if (mustKnowItems.length === 0) {
    const initialMustKnow = [
      {
        id: 'mk-discount',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'DISCOUNT & PREMIUM',
        icon: '💰',
        description: "Today's premium may not be tomorrow's premium. Discounts may change or be removed as per policy terms.",
        paragraphs: [
          "Today's premium may not be tomorrow's premium.",
          'Discounts may change or be removed as per the policy terms.'
        ],
        display_order: 1,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'mk-room',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'ROOM CATEGORY',
        icon: '🏥',
        description: 'Check your eligible room category before buying. Don’t assume “Any Room” means every room category is available without conditions.',
        paragraphs: [
          'Check your eligible room category before buying.',
          'Don’t assume “Any Room” means every room category is available without conditions.'
        ],
        display_order: 2,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'mk-health',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'HEALTH CHECK-UP',
        icon: '🩺',
        description: "Free doesn't always mean anytime. The benefit is available at renewal, subject to policy terms.",
        paragraphs: [
          "Free doesn't always mean anytime.",
          'The benefit is available at renewal, subject to the policy terms.'
        ],
        display_order: 3,
        status: 'active',
        created_at: now,
        updated_at: now
      },
      {
        id: 'mk-cataract',
        plan_id: 'hdfc-optima-secure-plus',
        title: 'CATARACT',
        icon: '👁️',
        description: "No amount limit doesn't necessarily mean every lens is covered. Check eligible lens category before buying.",
        paragraphs: [
          "No amount limit doesn't necessarily mean every lens is covered.",
          'Check the eligible lens category before relying on this benefit.'
        ],
        display_order: 4,
        status: 'active',
        created_at: now,
        updated_at: now
      }
    ];
    db.mustKnow.seedInitial(initialMustKnow);
    console.log(`Seeded ${initialMustKnow.length} Must Know items for Optima Secure+.`);
  }
}

// Run directly if invoked from CLI
if (process.argv[1] && (process.argv[1].endsWith('seed.js') || process.argv[1].includes('seed.js'))) {
  seedDatabase();
  console.log('Database seeding completed.');
}
